...
class SolverBfs:
    ...
    def solve(self, max_d) -> Node:
        self.max_d = max_d

        frontier = deque()
        frontier.append(self.world.start_node)# collezioni di nodi da cui vengono estratti elementi con logica fifo
        reached = {self.world.start_node.state} #set of exanded nodes
        nods_in_frontier = {self.world.start_node} #set of nodes currently in the frontier
        ...
        while frontier: #finche la frontiera non vuota
            ...
            for child in self.expand(n):
                ...
                if  (not child.state in reached) and (not child in nods_in_frontier): 
                    ...
                    #lo aggiungiamo a reached e alla frontiera
